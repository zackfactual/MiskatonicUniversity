...
namespace MiskatonicUniversity.Controllers
{
    public class DepartmentController : Controller
    {
        ...
        public async Task<ActionResult> Details(int? id)
        {
			...
			// commenting out original code to use a raw SQL query
        ...
        public async Task<ActionResult> Edit(int? id, byte[] rowVersion)
		{
			...
			if (TryUpdateModel(departmentToUpdate, fieldsToBind))
			{
				...
				// if no rows are affected by the UPDATE command (no rows have the original RowVersion value)
					// EF throws an exception
				catch (DbUpdateConcurrencyException ex)
				{
					// retrieve affected Department entity from exception object
					var entry = ex.Entries.Single();
					// get new values entered by user
					var clientValues = (Department)entry.Entity;
					// get values read from the dB
					...
					// create custom error message for columns with dB values different from user entry values
					else
					{
						...
						// set RowVersion value of Department object to value retrieved from dB
						departmentToUpdate.RowVersion = databaseValues.RowVersion;
					}
				}
				catch (RetryLimitExceededException /* dex */)
				{
					// log the error (uncomment dex variable name and add line here to write a log)
					ModelState.AddModelError("", "Unable to save changes. Try again, and if the problem persists, see your system administrator.");
				}
			}
			ViewBag.InstructorID = new SelectList(db.Instructors, "ID", "FullName", departmentToUpdate.InstructorID);
			// redisplay edit page for user
        ...
        public async Task<ActionResult> Delete(int? id, bool? concurrencyError)
        {
			...
			//  if the page is redisplayed following a currency error, give the user an error message
			if (concurrencyError.GetValueOrDefault())
			{
				ViewBag.ConcurrencyErrorMessage = "The record you attempted to delete was modified by another user after you got the original values. "
					+ "The delete operation was canceled and the current valuesin the database have been displayed. "
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
		// by accepting a department instead of a record id as a parameter,
			//you can access the RowVersion property value in addition to the record key
        public async Task<ActionResult> Delete(Department department)
        {
			...
			// if a concurrency error is caught
			catch (DbUpdateConcurrencyException)
			{
				// redisplay the Delete confirmation page with a concurrency error message
